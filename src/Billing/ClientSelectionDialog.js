...
const ClientSelectionDialog = ({ open, onClose, charLimit = 4000,handleDrawerClose }) => {
    ...
    fetch(url, requestOptions)
      ...
      .then((result) => {
        ...
        if (result && result.message === "Invoice created successfully") {
          ...
          setDrawerOpen(false)
          handleDrawerClose()
          navigate(`/clients/accounts/accountsdash/invoices/${selectedAccount.value}/invoice`)
          // handleClose();
          fetchInvoiceData();
        } else {
    ...
    <Box>
      ...
      <Box>
        <FormControl fullWidth>
          ...
          <LocalizationProvider dateAdapter={AdapterDayjs}>
            <DatePicker
              value={startDate} // Set default value
              onChange={handleStartDateChange}
              format="DD/MM/YYYY"
              sx={{ width: "100%", backgroundColor: "#fff" }}
          ...
          <Box sx={{ pt: 3, display: "flex", alignItems: "center", gap: 5 }}>
                          <Button
                            variant="contained"
                            color="primary"
