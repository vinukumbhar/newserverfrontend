...
const getAccountData = () => {
  let config = {
    ...
    url: `${ACCOUNT_API}/accounts/accountdetails/${data}`,
    headers: {}
  };
  ...
  axios.request(config)
    .then((response) => {
      ...
      setAccountData(response.data); // <-- Save the data to state
      setAccountEmail(response.data.account.contacts[0].email)
      console.log("account email",response.data.account.contacts[0].email);
    })
    .catch((error) => {
      console.log(error);
    });
  ...
  useEffect(() => {
    if (data) { // Make sure `data` is available before calling
      getAccountData();
    }
  }, [data]);
  ...
  const fetchEmails = useCallback(() => {
    ...
    gapi.client.gmail.users.messages.list({
      ...
      Promise.all(emailPromises).then(emailData => {
        ...
        // // ⭐ FILTER: Only show emails FROM your account email
        const filteredEmails = processedEmails.filter(email =>
          email.from && email.from.includes(accountEmail) // Match your own account email
        );
        // ⭐ FILTER: Only show emails where TO == your account email
// const filteredEmails = processedEmails.filter(email =>
//   email.to && email.to.includes(accountEmail)
// );

  
        setEmails(filteredEmails);
        console.log("Filtered Inbox Emails:", filteredEmails);
        setLoading(false);
      }).catch(error => {
        console.error("Error fetching individual emails:", error);
        setLoading(false);
  ...
  const fetchSentEmails = useCallback(() => {
    ...
    gapi.client.gmail.users.messages.list({
      ...
      Promise.all(emailPromises).then(emailData => {
        ...
        // ⭐ FILTER: Only show emails SENT to your account email
        const filteredSentEmails = processedEmails.filter(email =>
          email.to && email.to.includes(accountEmail) // Match your own account email
        );
  
        setSentEmails(filteredSentEmails);
        console.log("Filtered Sent Emails:", filteredSentEmails);
        setLoading(false);
      }).catch(error => {
        console.error("Error fetching individual sent emails:", error);
        setLoading(false);
