...
//                     <Typography variant="h1" sx={{ fontSize: '38px', marginBottom: '5%' }}>Reset Password</Typography>
//                 </Box>
...
const ResetPassword = () => {
  ...
  const getUser = () => {
    ...
    fetch(`${ACCOUNT_API}/accounts/accountsuserid/${data}`, requestOptions)
      .then((response) => response.json())
      .then((result) => {
        console.log(result);
        setUserid(result.account.userid);
      })
      .catch((error) => console.error(error));
  };
  useEffect(() => {
...
//     // Get user ID from localStorage or context (update this part based on your app)
...
//         `${LOGIN_API}/common/user/password/updateuserpasswordwithoutauth/`,
//         {
//           password: password,
//         },
//         {
    ...
    try {
      const response = await axios.patch(
        ...
        },
        {
          headers: {
            "Content-Type": "application/json",
            id: userid, // Use userid from getUser
          },
        }
      );
